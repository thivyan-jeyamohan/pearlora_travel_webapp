...
export default function AirTravel() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate destination time (must be after departure time)
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/airtaxitravels", formData);
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row items-center bg-white shadow-xl rounded-2xl p-6 max-w-5xl">
      ...
      <div className="hidden md:flex w-1/2">
        <img src={air} alt="Airplane" className="rounded-xl w-full h-full object-cover" />
      </div>
