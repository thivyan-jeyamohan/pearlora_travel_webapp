        ...
        {travels.map((travel) => (
          <div key={travel._id} className="bg-white shadow-lg rounded-lg p-6 w-[400px] border-2 border-gray-300 relative">
            ...
            <div className="flex justify-center mt-5 space-x-2">
              ...
              <button onClick={() => handleEdit(travel)} className="bg-violet-800 text-white rounded-xl font-bold px-6 py-2">
              <FiEdit />
              </button>
              <button onClick={() => handleDelete(travel._id)} className="bg-violet-950 text-white rounded-xl font-bold px-6 py-2">
              <MdDeleteOutline />
            ...
            {editingTravel === travel._id && (
              <div className="absolute top-0 left-0 w-full h-full rounded-lg bg-white shadow-xl p-4 z-10">
                ...
                <form onSubmit={handleUpdate}>
                  ...
                  <input type="text" value={formData.airtaxiName} 
                      onChange={(e) => setFormData({...formData, airtaxiName: e.target.value})} 
                  ...
                  <div className="flex mt-4 justify-center gap-1">
                    <button type="submit" className="bg-blue-800 text-white px-4 py-2 rounded-full "><FaSave /></button>
                    <button onClick={() => setEditingTravel(null)} className="ml-2 bg-red-700 text-white px-4 py-2 rounded-full"><MdCancel /></button>
                  </div>
