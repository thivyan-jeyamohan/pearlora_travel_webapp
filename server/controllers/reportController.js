...
export const getReport = async (req, res) => {
    try {
        ...
        if (req.query.checkInDateTo) {
            query.checkInDate = { ...query.checkInDate, $lte: new Date(req.query.checkInDateTo) };
        }
        if (req.query.checkOutDateFrom) {
            query.checkOutDate = { $gte: new Date(req.query.checkOutDateFrom) };
        }
        if (req.query.checkOutDateTo) {
            query.checkOutDate = { ...query.checkOutDate, $lte: new Date(req.query.checkOutDateTo) };
        }
        ...
        const reportData = await Promise.all(bookings.map(async (booking) => {
            ...
            const hotel = room ? await Hotel.findById(room.hotelId) : null;
